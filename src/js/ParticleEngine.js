...
ParticleEngine.prototype.createParticle = function()
{
	...
	// opacity tween attached per article
  ...
  // modify size and particle based on smoke intensity
  // did this break the FPS?
  if (this.intensity) {
    particle.size = this.randomValue( this.sizeBase, this.intensity );
  } else {
...
function updateMaterialAttributes(material, particle, i) {
  material.attributes.customVisible.value[i] = particle.alive;
  material.attributes.customColor.value[i]   = particle.color;
  material.attributes.customOpacity.value[i] = particle.opacity;
  material.attributes.customSize.value[i]    = particle.size;
  material.attributes.customAngle.value[i]   = particle.angle;
}

ParticleEngine.prototype.update = function(dt, intensity)
{
	var recycleIndices = [];

  if (intensity) {
    this.intensity = intensity;
    // scale is located in ParticleEngineMeta

    // TODO add more particles without breaking

    this.particleCount = intensity * this.particleCountIntensityScale;
  }
